# Инструкция
Укажите в start.tsv построчно: x-координату, y-координату, кол-во песчинок.
Каждая строчка - отдельная точка с песчинками.
## Запуск в консоли
Пример:
```labwork3_bimo0o.exe -i start.tsv -o <желаемое название bmp>```
...
Результат будет лежать в BMP_PHOTOS
## ВАЖНО!
цвета плохо работают на WINDOWS 11 (обратный порядок) 
так что на WINDOWS 11 все будет голубое :(
